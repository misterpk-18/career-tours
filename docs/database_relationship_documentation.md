...
**Note:** Every join in the matching engine — `occupation_skills`, `course_skills`, `student_skills` — resolves on `skill_id` **exactly**; nothing anywhere matches fuzzily. So a skill only participates in matching if every producer spells it the same way, which is what `services/skills/taxonomy.py` and `normalizer.py` exist to guarantee. Deleting from this table is also more dangerous than it looks: it cascades into `occupation_skills`, `student_skills` and `career_skill_gaps`, and `project_skills` holds a NO ACTION reference that makes a bare `DELETE FROM skills` fail outright.
